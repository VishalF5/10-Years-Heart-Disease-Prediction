...
def SameValuesCols(df, lExclCols=[]):
    #lExclCols = depVars
    # preparing for standadrising
    # orig col names
    colNames = df.columns.tolist()
    ...
    lRetVals = []
    for colName in colNames:
        if df[colName].std() == 0 and df[colName].var() == 0:
            lRetVals.append(colName)
    return lRetVals

# identify columns with more than 100% unique values
def UniqValuesCols(df, lExclCols=[], Percent=0.95):
    #lExclCols = depVars
    # preparing for standadrising
    # orig col names
    colNames = df.columns.tolist()
    ...
    for colName in colNames:
        ...
        if cntUniq/cntRecs >= Percent:
            lRetVals.append(colName)
    return lRetVals

# identify columns with more than 50% null values
def NullValuesCols(df, lExclCols=[], Percent=0.5):
    #lExclCols = depVars
    # preparing for standadrising
    # orig col names
    colNames = df.columns.tolist()
